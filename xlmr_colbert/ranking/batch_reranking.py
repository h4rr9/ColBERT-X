...
def prepare_ranges(index_path, dim, step, part_range):
    ...
    def _loader_thread(index_path, dim, positions):
        for offset, endpos in positions:
            index = IndexPart(
                index_path, dim=dim, part_range=range(offset, endpos), verbose=True
            )
...
def score_by_range(
    positions, loaded_parts, all_query_embeddings, all_query_rankings, all_pids
):
    ...
    for offset, endpos in positions:
        ...
        range_start = (
            range_start + (all_pids[range_start:] < index.pids_range.start).sum()
        )
        ...
        print_message(
            f"#> Ranking in batches the pairs #{range_start} through #{range_end}..."
        )
        scores = index.batch_rank(
            all_query_embeddings, query_indexes, pids, sorted_pids=True
        )

        for query_index, pid, score in zip(
            query_indexes.tolist(), pids.tolist(), scores
        ):
...
def batch_rerank(args):
    ...
    with torch.no_grad():
        ...
        all_query_embeddings = inference.queryFromText(
            queries_in_order, bsize=512, to_cpu=True
        )
    ...
    all_pids = flatten(
        [
            [(query_index, pid) for pid in topK_pids[qid]]
        ...
        with torch.no_grad():
            for query_index, qid in enumerate(queries):
                if query_index % 1000 == 0:
                    print_message(
                        "#> Logging query #{} (qid {}) now...".format(query_index, qid)
                    )
